...
def run_bot(network: str):
    ...
    account = Account(os.getenv("acct_address"), key, "pqd-v")
    ...
    while (True):
        ...
        try:
            cursor.execute("SELECT id, asset1_id, asset2_id, asset_in_id, asset_in_amt, slippage, min_price_for_sell, do_redeem, do_reverse FROM trades WHERE wallet_address=? AND token_network=? AND is_completed=?", (account.account_info, token_network, 0))
        ...
        if token_network == "testnet":
            algod = get_algod()
            client = TinymanTestnetClient(algod_client=account)
        else:
